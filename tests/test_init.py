...
async def test_setup_unload_and_reload_entry(hass):
    ...
    with patch(
        ...
        assert type(hass.data[DOMAIN][config_entry.unique_id]) == RenaultHub
...
async def test_setup_entry_bad_password(hass):
    ...
    with patch(
        "custom_components.renault.RenaultHub.attempt_login", return_value=False
    ):
        # Set up the entry and assert that the values set during setup are where we expect
        # them to be.
...
async def test_setup_entry_exception(hass):
    ...
    # ConfigEntryNotReady.
    with patch(
        "custom_components.renault.RenaultHub.attempt_login",
